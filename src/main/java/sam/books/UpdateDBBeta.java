...
public class UpdateDBBeta implements Callable<Boolean> {
	private Logger logger = LoggerFactory.getLogger(UpdateDBBeta.class);
	...
	private void walk(Iterable<? extends FileWrap> children, Consumer<FileWrap> consumer) {
	...
	private List<Dir> load(Path updatePath) throws IOException {
		List<FileWrap> allWraps = new WrapSerializer().read(updatePath);

		if(Checker.isEmpty(allWraps))
			return walk();

		ArrayList<FileWrap> modified = new ArrayList<>();
		ArrayList<FileWrap> notFound = new ArrayList<>();
		...
		allWraps.forEach(w -> {
			...
			if(!w.exists())
				notFound.add(w);
			else if(w.isModified())
				modified.add(w);
		});

		if(modified.isEmpty() && notFound.isEmpty()){
			logger.info("no chages in dirs");
			return null;
		}

		Map<Path, FileWrap> map = allWraps.stream().collect(Collectors.toMap(FileWrap::subpath, w -> w));
		ArrayList<FileWrap> newDirs = new ArrayList<>();
		ArrayList<FileWrap> newFiles = new ArrayList<>();
		Set<Path> skipfiles = skipFiles();
		int count = ROOT.getNameCount();
		List<FileWrap> modified2 = new ArrayList<>();

		Set<Path> walked = new HashSet<>();

		for (FileWrap w : modified) {
			if(walked.contains(w.path()))
				continue;

			/* FIXME
			 * 	Files.walkFileTree(w.path(), new SimpleFileVisitor<Path>() {
				public FileVisitResult preVisitDirectory(Path dir, BasicFileAttributes attrs) throws IOException {
					...
					walked.add(dir);
					return process(dir, attrs, true);
				}
				public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
					return process(file, attrs, false);
				};
				private FileVisitResult process(Path file, BasicFileAttributes attrs, boolean isDir) {
					Path subpath = file.subpath(count, file.getNameCount());

					if(skipfiles.contains(subpath)){
						logger.debug("skip: "+subpath);
						return isDir ? FileVisitResult.SKIP_SUBTREE : FileVisitResult.CONTINUE;
					}
					FileWrap w = map.get(subpath);

					if(w == null) {
						(isDir ? newDirs : newFiles).add(new FileWrap(file, subpath, isDir, attrs.lastModifiedTime().toMillis()));
						logger.info("new: "+subpath);
						return FileVisitResult.CONTINUE;
					} else if(w.isDir()) {
						if(w.isModified()) {
							logger.info("modified: "+subpath);
							modified2.add(w);
						} else 
	...
	private List<Dir> walk() throws IOException {
		...
		for (String s : root.list()) {
			...
			if(skipfiles.contains(subpath) || !Files.isDirectory(fullpath)) 
				...
				Map<Path, FileWrap> map = new HashMap<>();
				walk(map, fullpath, subpath, skipfiles);
				dirs.add(new Dir(fullpath, subpath, Files.getLastModifiedTime(fullpath).toMillis(), map));
			}
		} 
		return dirs;
	}
	private void walk(Map<Path, FileWrap> parent_children, Path fullpath, Path subpath, Set<Path> skipFiles) throws IOException {
		for (String s : fullpath.toFile().list()) {
			final Path p = Paths.get(s);
			final Path sp = subpath.resolve(p);
			...
			final Path fp = fullpath.resolve(p);
			...
			if(attrs.isDirectory()) {
				Map<Path, FileWrap> map = new HashMap<>();
				f = new Dir(fp, sp, attrs.lastModifiedTime().toMillis(), map);
				walk(map, fp, sp, skipFiles);
			} else  {
				f = new FileWrap(fp, sp, attrs.lastModifiedTime().toMillis());
			}
			parent_children.put(p, f);
		}
	}

	private Set<Path> skipFiles() throws IOException {
		Path ignoreFilesPath = SELF_DIR.resolve("ignore-subpaths.txt");
		Set<Path> skipfiles = Files.notExists(ignoreFilesPath) ? Collections.emptySet() : Files.lines(ignoreFilesPath).filter(Checker::isNotEmptyTrimmed).filter(s -> s.charAt(0) != '#').map(Paths::get).collect(Collectors.collectingAndThen(Collectors.toList(), s -> {
			if(s.isEmpty())
